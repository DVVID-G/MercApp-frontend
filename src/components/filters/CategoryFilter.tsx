...
export const CategoryFilter: React.FC = () => {
  ...
  const selectedCount = state.categories.length;

  const handleToggleCategory = (category: string) => {
    const isSelected = state.categories.includes(category);
    if (isSelected) {
      dispatch({ type: 'setCategories', payload: state.categories.filter((c) => c !== category) });
    } else {
      dispatch({ type: 'setCategories', payload: [...state.categories, category] });
    }
  };

  return (
    <div className="space-y-2" role="group" aria-labelledby="category-filter-title">
      <button
        type="button"
              ...
              {CATEGORIES.map((category) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-sm text-gray-300 font-medium">{CATEGORY_LABELS[category]}</span>
                  </button>
                );
              })}
            </div>
